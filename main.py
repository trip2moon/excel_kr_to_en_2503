...
async def translate_ko_to_en(text, src='ko', dest='en'):
    text_translated = ''
...
def process_translation_by_dir(dir_excel_target, reference_dict):
    ...
    for file_one in file_list:
        if file_one.endswith(FILE_EXCEL_EXT) is True:
            ...
            asyncio.run(process_translation(file_path, reference_dict))

async def process_translation(file_path_input, reference_dict):
    ...
    for sheet_name in wb.sheetnames:
        ...
        # ws_output = wb.create_sheet(sheet_name_new)
        ...
        for row in ws_input.iter_rows(min_row=ws_input.min_row, max_row=ws_input.max_row):
            for cell in row:
                ...
                if value_str is not None:
                    ...
                    reference_found, translated = check_reference(value_str, reference_dict)
                    if reference_found is False:
                        ...
                        reference_dict[value_str] = translated
                    ...
                    new_cell = ws_output.cell(row=cell.row, column=cell.col_idx,
                                          value=translated)
                    if cell.has_style:
...
if __name__ == '__main__':
    ...
    # text_ex_01 = 'AI기술개발 지원 및 AI솔루션 확산을 위한 제조AI오픈랩 구축·운영 및 AI솔루션 도입 어려움 해소를 위한 AI솔루션 도입 컨설팅'
    # print(f"Input :{text_ex_01}")
    # asyncio.run(translate_ko_to_en(text_ex_01))